...
export default function CanvasCamera5Component() {
    ...
    const { gestureMode, cameraActive, setCameraActive,  setStartHandPoseDetection , startHandPoseDetection } = AppContext;
      ...
      appState === "init" && 
          ...
          if (videoRef.current) {
            if (videoRef.current.elt.srcObject) {
              videoRef.current.elt.srcObject.getTracks().forEach(track => track.stop());
            }
    ...
    return (
        ...
        {!isML5Loaded && (
          <div className="library-loading">Cargando librerías ML5JS y activando cámara...</div>
        )}
        </>
    );
}